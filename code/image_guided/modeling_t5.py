...
class JointEncoder(T5Stack):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            assert self.embed_tokens is not None, "You must initialize the model with valid token embeddings"
        ...
        # We must make it broadcastable to all heads.
        ...
        if self.config.num_layers > 0:
            ...
            for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
                ...
                # We share the position biases between the layers and store them
                # in the first layer.
...
class VLT5(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # If decoding with past key value states, only the last tokens should 
